...
```{r}
# pbmc_10x_multiome("mouse_gastrulation", mode = "*", version = "1.0.0", dry.run = TRUE)
# scMultiome("pbmc", mode = "*", version = "1.0.0", dry.run = TRUE, format = "mtx")
mae <- readRDS("/Users/ricard/data/multiome_10x_public_data/PBMC/MultiAssayExperiment/mae.rds")
```
...
```{r}
all(colnames(experiments(mae)[["rna"]]) == colnames(experiments(mae)[["atac"]]))
# upsetSamples(mae)
```
